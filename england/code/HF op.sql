...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'hf_op_0622'
...
-----------------------------------------------
-- HF procedure episodes
-----------------------------------------------
-- This query returns one row per each HF procedure for each episode - this means there are multiple rows per episode!!!
...
(LEFT(code, 4) IN ('K541', 'K542') OR LEFT(code, 3) IN ('K01', 'K02', 'K59', 'K60', 'K61'))
...
base.fae,
base.classpat,
IF(tretspef = 192, 1, 0) AS ITU_Admission,
...
CASE
  WHEN LEFT(icd, 4) IN ('I500', 'I501', 'I502', 'I503', 'I504') THEN 1
  WHEN LEFT(icd, 4) IN ('I509') THEN 2
  ELSE 0
  END AS GROUPED_DIAG_epi,
CASE
  WHEN LEFT(icd, 4) IN ('I500', 'I501', 'I502', 'I503', 'I504') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I509') THEN pos_icd
  ELSE 0
...
PERSON_ID, SPELL_ID, EPIKEY,
CASE
  WHEN LEFT(op, 3) = 'K01' THEN 5
  WHEN LEFT(op, 3) = 'K02' THEN 4
  WHEN LEFT(op, 4) IN ('K541', 'K542') THEN 3
  WHEN LEFT(op, 3) IN ('K59') THEN 2
  WHEN LEFT(op, 3) IN ('K60', 'K61') THEN 1 
  ELSE 0
...
COALESCE(ITU_Admission.ITU_Admission, 0) AS ADMISSION_TO_ITU,
--Charlson
(
  ...
  --Charlson.CHARLSON_CONGEST_HEART_FAIL*1+ --removed for this spec
  ...
  Charlson.CHARLSON_CANCER*2+
  Charlson.CHARLSON_SEVERE_LIVER*3+
  Charlson.CHARLSON_HIV*6+
  Charlson.CHARLSON_METASTATIC_CANCER*6
) AS CHARLSON_SCORE,
...
LEFT OUTER JOIN
(--derived columns episodes table for ITU admission
  SELECT PERSON_ID, SPELL_ID, MAX(ITU_Admission) AS ITU_Admission
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes WHERE ITU_Admission > 0 GROUP BY PERSON_ID, SPELL_ID
) AS ITU_Admission ON base.PERSON_ID = ITU_Admission.PERSON_ID AND base.SPELL_ID = ITU_Admission.SPELL_ID
...
select * from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base ORDER BY PERSON_ID, SpellOrder
...
select * FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells
...
CREATE TABLE IF NOT EXISTS .ccu003_04_nh_final_admissions_${ccu003_04_spec} AS 
  ...
  --Grouped dx/op fields
  GROUPED_DIAGNOSIS_ANY AS HF_DIAGNOSIS_ANY,
  GROUPED_DIAGNOSIS_ANY_POSITION AS HF_DIAGNOSIS_ANY_POSITION,
  ...
  GROUPED_PROCEDURE AS HEART_FAILURE_PROCEDURE,  
...
select year(admidate), count(*) from .ccu003_04_nh_final_hf_admissions_op_old group by year(admidate) order by year(admidate)
