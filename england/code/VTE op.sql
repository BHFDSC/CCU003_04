...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'vte_op'
...
-----------------------------------------------
-- VTE procedures episodes
-----------------------------------------------
-- This query returns one row per each VTE procedure for each episode - this means there are multiple rows per episode!!!
...
(LEFT(code, 4) IN ('L124', 'L125') OR LEFT(code, 3) = 'L13')
...
CASE
  WHEN LEFT(icd, 3) = 'I26' THEN 1
  WHEN LEFT(icd, 3) = 'I81' THEN 2
  WHEN LEFT(icd, 3) = 'I82' THEN 3
  WHEN LEFT(icd, 3) = 'I80' THEN 4
  ELSE 0
  END AS GROUPED_DIAG_epi,
CASE
  WHEN LEFT(icd, 3) = 'I26' THEN pos_icd
  WHEN LEFT(icd, 3) = 'I81' THEN pos_icd
  WHEN LEFT(icd, 3) = 'I82' THEN pos_icd
  WHEN LEFT(icd, 3) = 'I80' THEN pos_icd
  ELSE 0
...
DROP TABLE IF EXISTS .ccu003_04_kmc_final_vte_admissions_op_v1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_vte_admissions_op_v1 AS 
  ...
  --GROUPED_DIAGNOSIS_PRIMARY AS VTE_DIAGNOSIS_PRIMARY,
  PRIMARY_DISCHARGE_DIAGNOSIS_CODE,
  GROUPED_PROCEDURE AS VTE_PROCEDURE,  
...
ALTER TABLE .ccu003_04_kmc_final_vte_admissions_op_v1 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_kmc_final_vte_admissions_op_v1

-- COMMAND ----------

refresh table .ccu003_04_kmc_final_vte_admissions_op

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_vte_admissions_op

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_vte_admissions_op_v1

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_kmc_final_vte_admissions_op

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_kmc_final_vte_admissions_op_v1

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_kmc_final_vte_admissions_op

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_kmc_final_vte_admissions_op_v1

-- COMMAND ----------

select count(distinct procodet) from .ccu003_04_kmc_final_vte_admissions_op 

-- COMMAND ----------

select count(distinct procodet) from .ccu003_04_kmc_final_vte_admissions_op_v1 
