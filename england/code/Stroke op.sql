...
DROP TABLE IF EXISTS .ccu003_04_nh_temp_episodes_base

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_episodes_base AS
...
ALTER TABLE .ccu003_04_nh_temp_episodes_base OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_04_nh_temp_episodes_base

-- COMMAND ----------

select * from .ccu003_04_nh_temp_episodes_base order by PERSON_ID, SPELL_ID, EPIKEY
...
( -- CTE where the ICD fields are exploded into rows - one row for each diagnosis in each episode
  ...
  FROM .ccu003_04_nh_temp_episodes_base
)
...
REFRESH TABLE .ccu003_04_nh_temp_episodes_base

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_04_nh_temp_final_episodes1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_final_episodes1 AS
  ...
  FROM .ccu003_04_nh_temp_episodes_base AS base
  --derived grouped dx
  LEFT OUTER JOIN  
  (
...
ALTER TABLE .ccu003_04_nh_temp_final_episodes1 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_04_nh_temp_final_episodes1

-- COMMAND ----------

select * from .ccu003_04_nh_temp_final_episodes1
...
DROP TABLE IF EXISTS .ccu003_04_nh_temp_spells_base9

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_spells_base9 AS
  ...
  FROM
  (--base spells
    SELECT PERSON_ID, SPELL_ID, MIN(Epikey) AS AdmissionEpikey, MAX(Epikey) AS DischargeEpikey, COUNT(*) AS EpisodeCount FROM .ccu003_04_nh_temp_final_episodes1 GROUP BY PERSON_ID, SPELL_ID
  ) AS base_spells
  --admission fields
  INNER JOIN .ccu003_04_nh_temp_final_episodes1 AS admission_episodes ON base_spells.AdmissionEpikey = admission_episodes.Epikey
  --discharge fields
  INNER JOIN .ccu003_04_nh_temp_final_episodes1 AS discharge_episodes ON base_spells.DischargeEpikey = discharge_episodes.Epikey
  ...
  (
    ...
    FROM .ccu003_04_nh_temp_final_episodes1 AS a INNER JOIN
    (--first find the min hierarchical code. Once found, find the minimum position for it
      SELECT PERSON_ID, SPELL_ID, MIN(Grouped_diag) AS Grouped_diag_epi
      FROM .ccu003_04_nh_temp_final_episodes1 WHERE Grouped_diag > 0 GROUP BY PERSON_ID, SPELL_ID
  ...
  (
    ...
    (--first find the min hierarchical code. Once found, find the minimum position for it
      ...
      FROM .ccu003_04_nh_temp_final_episodes1 WHERE Grouped_diag > 0 GROUP BY EPIKEY
  ...
  (--find the min hierarchical code for position = 1
    SELECT EPIKEY, MIN(Grouped_diag) AS Grouped_diag_epi
    FROM .ccu003_04_nh_temp_final_episodes1 WHERE Grouped_diag > 0 AND Grouped_diag_pos = 1 GROUP BY EPIKEY
  ...
  (
    ...
    FROM .ccu003_04_nh_temp_final_episodes1 WHERE Grouped_proc > 0 GROUP BY PERSON_ID, SPELL_ID
  ) AS any_derived_op ON base_spells.PERSON_ID = any_derived_op.PERSON_ID AND base_spells.SPELL_ID = any_derived_op.SPELL_ID

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_temp_spells_base9 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

refresh table .ccu003_04_nh_temp_final_episodes1

-- COMMAND ----------

select * from .ccu003_04_nh_temp_spells_base9 where Grouped_diag_pri = 0

-- COMMAND ----------

select * from .ccu003_04_nh_temp_final_episodes1 where spell_id = 4901218925

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_04_nh_temp_final_spells
...
CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_final_spells AS
...
FROM .ccu003_04_nh_temp_spells_base9 AS base 
...
(--base episodes table for died
  ...
  FROM .ccu003_04_nh_temp_final_episodes1 WHERE died > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for stroke presence of Atrial Fibrillation
  ...
  FROM global_temp.nh_temp_episodes_diagcols WHERE PRESENCE_OF_AFIB > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for stroke presence of acute stroke 'I63'
  ...
  FROM global_temp.nh_temp_episodes_diagcols WHERE CEREBRAL_INFARCT > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for treatment with IV thombolysis
  ...
  FROM global_temp.nh_temp_episodes_opercols WHERE THROMBOLYSIS_INT > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for thrombectomy
  ...
  FROM global_temp.nh_temp_episodes_opercols WHERE THROMBECTOMY_INT > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for cerebral aneurysm coiling item 1
  ...
  FROM global_temp.nh_temp_episodes_opercols WHERE COILING_1 > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for cerebral aneurysm coiling item 2
  ...
  FROM global_temp.nh_temp_episodes_opercols WHERE COILING_2 > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for cerebral aneurysm coiling item 3
  ...
  FROM global_temp.nh_temp_episodes_opercols WHERE COILING_3 > 0 GROUP BY PERSON_ID, SPELL_ID
...
ALTER TABLE .ccu003_04_nh_temp_final_spells OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_nh_temp_final_spells where GROUPED_DIAGNOSIS_PRIMARY > 0
...
WITH cte AS
(SELECT PERSON_ID, SPELL_ID, Admidate, Disdate, Disdest, Dismeth, SPELL_ID, Admidate, Disdate, Admimeth, Dismeth, PRIMARY_DISCHARGE_DIAGNOSIS_CODE, ROW_NUMBER() OVER (PARTITION BY PERSON_ID ORDER BY Admidate, Disdate, SPELL_ID) AS RowNo FROM .ccu003_04_nh_temp_final_spells WHERE SPELL_ID IS NOT NULL)
...
REFRESH TABLE .ccu003_04_nh_temp_final_spells

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_04_nh_temp_superspells_base1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_superspells_base1 AS
...
ALTER TABLE .ccu003_04_nh_temp_superspells_base1 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_nh_temp_superspells_base1 ORDER BY PERSON_ID, SpellOrder
...
DROP TABLE IF EXISTS .ccu003_04_nh_temp_superspells_lookup1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_superspells_lookup1 AS
...
FROM .ccu003_04_nh_temp_superspells_base1 WHERE SpellOrder = 1

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_temp_superspells_lookup1 OWNER TO `rmhikmc@ucl.ac.uk`
...
-- MAGIC FROM .ccu003_04_nh_temp_superspells_base1 AS base INNER JOIN .ccu003_04_nh_temp_superspells_lookup1 AS lookup ON base.Out_Spell_ID = lookup.Spell_ID AND base.PERSON_ID = lookup.Person_ID
-- MAGIC WHERE base.In_Spell_ID NOT IN (SELECT Spell_ID FROM .ccu003_04_nh_temp_superspells_lookup1 WHERE Person_ID = lookup.PERSON_ID AND Superspell_ID = lookup.Superspell_ID)"""
-- MAGIC 
-- MAGIC rowcount = 1
-- MAGIC loopcount = 1
-- MAGIC 
...
-- MAGIC   df.write.insertInto('.ccu003_04_nh_temp_superspells_lookup1', overwrite = False)

-- COMMAND ----------

SELECT * FROM .ccu003_04_nh_temp_superspells_lookup1 ORDER BY Person_ID, Superspell_ID, SpellNo

-- COMMAND ----------

select person_ID, superspell_ID, count(superspell_ID) from .ccu003_04_nh_temp_superspells_lookup1 group by person_ID, superspell_ID order by count(superspell_ID) desc, person_id asc

-- COMMAND ----------

select * from .ccu003_04_nh_temp_superspells_base1 where person_id = '317HPX57IRJ1KV4'

-- COMMAND ----------

select spellcount, count(spellcount) from
(select person_ID, superspell_ID, count(superspell_ID) as spellcount from .ccu003_04_nh_temp_superspells_lookup1 group by person_ID, superspell_ID)
group by spellcount
...
SELECT base.Person_ID, base.Superspell_ID, spells.Spell_ID, spells.Procodet, spells.Admidate, spells.Admiage, spells.Sex, Spells.Ethnos, Spells.Resgor_ons, spells.Admimeth FROM .ccu003_04_nh_temp_superspells_lookup1 AS base
INNER JOIN .ccu003_04_nh_temp_final_spells spells on base.Person_ID = spells.Person_ID and base.Spell_ID = spells.Spell_ID
...
SELECT base.* FROM .ccu003_04_nh_temp_superspells_lookup1 AS base
INNER JOIN
(
  SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_04_nh_temp_superspells_lookup1 GROUP BY Person_ID, Superspell_ID
) AS dis
...
(  
  ...
  FROM .ccu003_04_nh_temp_final_spells AS spells --to provide dx info
  INNER JOIN .ccu003_04_nh_temp_superspells_lookup1 AS base --to provide superspell info
...
select * from .ccu003_04_nh_temp_final_episodes1 where person_id = '317HPX57IRJ1KV4' order by spell_id, epiorder
...
REFRESH TABLE .ccu003_04_nh_temp_superspells_lookup1
...
DROP TABLE IF EXISTS .ccu003_04_nh_temp_final_superspells 

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_final_superspells AS
...
(--Admission spell
  ...
  FROM .ccu003_04_nh_temp_superspells_lookup1 AS base
  INNER JOIN .ccu003_04_nh_temp_final_spells AS spells
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
  WHERE base.SpellNo = 1
) AS admi
INNER JOIN
(--Discharge spell
  SELECT base.Disdate, base.Dismeth, base.Primary_dx, base.Person_ID, base.SuperSpell_ID FROM .ccu003_04_nh_temp_superspells_lookup1 AS base --these items have been carried through into the lookup for ease of access
  INNER JOIN
  (
    SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_04_nh_temp_superspells_lookup1 GROUP BY Person_ID, Superspell_ID --assumes SpellNo is applied in chronological order
  ) AS dis
  ON base.Person_ID = dis.Person_ID AND base.Superspell_ID = dis.Spell_ID AND base.SpellNo = dis.LastSpellNo
) AS dis
...
(--Derived columns, except grouped dx/op
  SELECT 
    ...
    MAX(CHARLSON_HIV) AS CHARLSON_HIV,
    MAX(CHARLSON_MILD_LIVER) AS CHARLSON_MILD_LIVER
  FROM .ccu003_04_nh_temp_final_spells AS spells
  INNER JOIN .ccu003_04_nh_temp_superspells_lookup1 AS base
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
...
ALTER TABLE .ccu003_04_nh_temp_final_superspells OWNER TO `rmhikmc@ucl.ac.uk`
...
DROP TABLE IF EXISTS .ccu003_04_kmc_final_stroke_admissions_op

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_stroke_admissions_op AS 
  ...
  CHARLSON_HIV,
  CHARLSON_MILD_LIVER
  FROM .ccu003_04_nh_temp_final_spells AS base
  FULL OUTER JOIN .ccu003_04_nh_temp_superspells_lookup1 AS lookup ON base.Person_ID = lookup.Person_ID AND base.Spell_ID = lookup.Spell_ID
  ...
  CHARLSON_HIV,
  CHARLSON_MILD_LIVER
  FROM .ccu003_04_nh_temp_final_superspells 
...
select * from .ccu003_04_kmc_final_stroke_admissions_op

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_stroke_admissions_op where CAROTID_PROCEDURE > 0
