...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'stroke_op'
...
-----------------------------------------------
-- Stroke procedure episodes
-----------------------------------------------
-- This query returns one row per each stroke procedure for each episode - this means there are multiple rows per episode!!!
...
(--Lateral views to incorporate 2 posexplodes. Where clause to link them
  SELECT PERSON_ID_DEID AS PERSON_ID, SUSSPELLID AS SPELL_ID, SITETRET, EPIORDER, EPIKEY, EPISTART, EPIEND, ADMIDATE, DISDATE, pos_op, op, pos_diag, diag
  ...
  LATERAL VIEW posexplode(array(diag_01, 	diag_02, 	diag_03, 	diag_04, 	diag_05, 	diag_06, 	diag_07, 	diag_08, 	diag_09, 	diag_10, 	diag_11, 	diag_12, 	diag_13, 	diag_14, 	diag_15, 	diag_16, 	diag_17, 	diag_18, 	diag_19, 	diag_20)) diags AS pos_diag, diag
  WHERE ops.pos_op = diags.pos_diag
  )
...
((LEFT(diag, 3) = 'I63' AND LEFT(op, 4) = 'X833') OR 
(LEFT(diag, 3) = 'I63' AND LEFT(op, 4) = 'L354') OR 
(LEFT(op, 3) IN('O01','O02','O03','O04') AND LEFT(op, 3) = 'Y53' AND LEFT (op, 3) = 'Z35') OR
LEFT(op, 4) IN ('L294','L295','L314'))
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base OWNER TO `rmhikmc@ucl.ac.uk`
...
CASE
  WHEN LEFT(icd, 3) IN ('I60', 'I61', 'I63', 'I64') THEN 1
  WHEN LEFT(icd, 3) = 'G45' THEN 2
  ELSE 0
  END AS GROUPED_DIAG_epi,
CASE
  WHEN LEFT(icd, 3) IN ('I60', 'I61', 'I63', 'I64') THEN pos_icd
  WHEN LEFT(icd, 3) = 'G45' THEN pos_icd
  ELSE 0
  END AS GROUPED_DIAG_epi_pos,
  
--Add any other grouped diagnostic fields here  
CASE
  WHEN LEFT(icd,3) = 'I48' THEN 1
ELSE 0
END AS PRESENCE_OF_AFIB,

CASE 
  WHEN LEFT(icd, 3) = 'I63' THEN 1 
ELSE 0 
END AS CEREBRAL_INFARCT
...
PERSON_ID, SPELL_ID, EPIKEY,
CASE
  WHEN LEFT(op, 4) = 'L314' THEN 2
  WHEN LEFT(op, 4) IN ('L294', 'L295') THEN 1 
  ELSE 0
  END AS GROUPED_PROC_epi,
--Add any other grouped procedural fields here  
CASE 
  WHEN LEFT(op, 3) IN ('O01','O02','O03','O04') THEN 1 
ELSE 0 
END AS COILING_1,

CASE 
  WHEN LEFT(op,3) = 'Y53' THEN 1 
ELSE 0 
END AS COILING_2,

CASE 
  WHEN LEFT(op,3) = 'Z35' THEN 1 
ELSE 0 
END AS COILING_3,

CASE 
  WHEN LEFT(op, 4) = 'X833' THEN 1
ELSE 0
END AS THROMBOLYSIS_INT,

CASE 
  WHEN LEFT(op, 4) = 'L354' THEN 1
ELSE 0
END AS THROMBECTOMY_INT  
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes OWNER TO `rmhikmc@ucl.ac.uk`
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base OWNER TO `rmhikmc@ucl.ac.uk`
...
COALESCE(presence_of_afib.PRESENCE_OF_AFIB, 0) AS PRESENCE_OF_AFIB,
COALESCE(cerebral_infarct.CEREBRAL_INFARCT, 0) AS CEREBRAL_INFARCT,
COALESCE(thrombolysis_int.THROMBOLYSIS_INT, 0) AS THROMBOLYSIS_INT,
COALESCE(thrombectomy_int.THROMBECTOMY_INT, 0) AS THROMBECTOMY_INT,
COALESCE(coiling_1.COILING_1, 0) AS COILING_1,
COALESCE(coiling_2.COILING_2, 0) AS COILING_2,
COALESCE(coiling_3.COILING_3, 0) AS COILING_3,
--Charlson
(
  Charlson.CHARLSON_CANCER*1+
  Charlson.CHARLSON_CEREBRO*0+
...
--Other grouped dx/op fields - add or remove as applicable
LEFT OUTER JOIN
(--derived columns episodes table for stroke presence of Atrial Fibrillation
  SELECT PERSON_ID, SPELL_ID, MAX(PRESENCE_OF_AFIB) AS PRESENCE_OF_AFIB
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_diagcols WHERE PRESENCE_OF_AFIB > 0 GROUP BY PERSON_ID, SPELL_ID
) AS PRESENCE_OF_AFIB ON base.PERSON_ID = PRESENCE_OF_AFIB.PERSON_ID AND base.SPELL_ID = PRESENCE_OF_AFIB.SPELL_ID

LEFT OUTER JOIN
(--derived columns episodes table for stroke presence of acute stroke 'I63'
  SELECT PERSON_ID, SPELL_ID, MAX(CEREBRAL_INFARCT) AS CEREBRAL_INFARCT
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_diagcols WHERE CEREBRAL_INFARCT > 0 GROUP BY PERSON_ID, SPELL_ID
) AS CEREBRAL_INFARCT ON base.PERSON_ID = CEREBRAL_INFARCT.PERSON_ID AND base.SPELL_ID = CEREBRAL_INFARCT.SPELL_ID

LEFT OUTER JOIN
(--derived columns episodes table for treatment with IV thombolysis
  SELECT PERSON_ID, SPELL_ID, MAX(THROMBOLYSIS_INT) AS THROMBOLYSIS_INT
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_opercols WHERE THROMBOLYSIS_INT > 0 GROUP BY PERSON_ID, SPELL_ID
) AS THROMBOLYSIS_INT ON base.PERSON_ID = THROMBOLYSIS_INT.PERSON_ID AND base.SPELL_ID = THROMBOLYSIS_INT.SPELL_ID

LEFT OUTER JOIN
(--derived columns episodes table for thrombectomy
  SELECT PERSON_ID, SPELL_ID, MAX(THROMBECTOMY_INT) AS THROMBECTOMY_INT
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_opercols WHERE THROMBECTOMY_INT > 0 GROUP BY PERSON_ID, SPELL_ID
) AS THROMBECTOMY_INT ON base.PERSON_ID = THROMBECTOMY_INT.PERSON_ID AND base.SPELL_ID = THROMBECTOMY_INT.SPELL_ID

LEFT OUTER JOIN
(--derived columns episodes table for cerebral aneurysm coiling item 1
  SELECT PERSON_ID, SPELL_ID, MAX(COILING_1) AS COILING_1
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_opercols WHERE COILING_1 > 0 GROUP BY PERSON_ID, SPELL_ID
) AS COILING_1 ON base.PERSON_ID = COILING_1.PERSON_ID AND base.SPELL_ID = COILING_1.SPELL_ID

LEFT OUTER JOIN
(--derived columns episodes table for cerebral aneurysm coiling item 2
  SELECT PERSON_ID, SPELL_ID, MAX(COILING_2) AS COILING_2
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_opercols WHERE COILING_2 > 0 GROUP BY PERSON_ID, SPELL_ID
) AS COILING_2 ON base.PERSON_ID = COILING_2.PERSON_ID AND base.SPELL_ID = COILING_2.SPELL_ID

LEFT OUTER JOIN
(--derived columns episodes table for cerebral aneurysm coiling item 3
  SELECT PERSON_ID, SPELL_ID, MAX(COILING_3) AS COILING_3
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_opercols WHERE COILING_3 > 0 GROUP BY PERSON_ID, SPELL_ID
) AS COILING_3 ON base.PERSON_ID = COILING_3.PERSON_ID AND base.SPELL_ID = COILING_3.SPELL_ID
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells OWNER TO `rmhikmc@ucl.ac.uk`
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base   OWNER TO `rmhikmc@ucl.ac.uk`
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup OWNER TO `rmhikmc@ucl.ac.uk`
...
(--Derived columns, except grouped dx/op
  SELECT 
    ...
    --Additional grouped dx/op fields - add or remove as applicable
    MAX(PRESENCE_OF_AFIB) AS PRESENCE_OF_AFIB,
    MAX(CEREBRAL_INFARCT) AS CEREBRAL_INFARCT,
    MAX(THROMBOLYSIS_INT) AS THROMBOLYSIS_INT,  
    MAX(THROMBECTOMY_INT) AS THROMBECTOMY_INT,
    MAX(COILING_1) AS COILING_1,
    MAX(COILING_2) AS COILING_2,
    MAX(COILING_3) AS COILING_3,
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_superspells OWNER TO `rmhikmc@ucl.ac.uk`
...
DROP TABLE IF EXISTS .ccu003_04_kmc_final_stroke_admissions_op_v1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_stroke_admissions_op_v1 AS 
  ...
  --Grouped dx/op fields
  GROUPED_DIAGNOSIS_ANY AS STROKE_DIAGNOSIS_ANY,
  GROUPED_DIAGNOSIS_ANY_POSITION AS STROKE_DIAGNOSIS_ANY_POSITION,
  GROUPED_DIAGNOSIS_DISCHARGE AS STROKE_DIAGNOSIS_DISCHARGE,
  GROUPED_DIAGNOSIS_DISCHARGE_POSITION AS STROKE_DIAGNOSIS_DISCHARGE_POSITION,
  GROUPED_DIAGNOSIS_PRIMARY AS STROKE_DIAGNOSIS_PRIMARY,
  PRIMARY_DISCHARGE_DIAGNOSIS_CODE,
  GROUPED_PROCEDURE AS CAROTID_PROCEDURE,  
  --Grouped cov / deaths fields
  POSS_COVID,
  DIED_DURING_ADMISSION,
  --Other grouped fields - add or remove as applicable
  PRESENCE_OF_AFIB,
  CASE WHEN THROMBOLYSIS_INT = 1 AND CEREBRAL_INFARCT = 1 THEN 1 ELSE 0 END AS THROMBOLYSIS_IS,
  CASE WHEN THROMBECTOMY_INT = 1 AND CEREBRAL_INFARCT = 1 THEN 1 ELSE 0 END AS THROMBECTOMY_IS,
  CASE WHEN COILING_1 = 1 AND COILING_2 = 1 AND COILING_3 = 1 THEN 1 ELSE 0 END AS COILING,
  --Charlson
  CHARLSON_SCORE,
  CHARLSON_CANCER,
  CHARLSON_CEREBRO,
...
select * from .ccu003_04_kmc_final_stroke_admissions_op_v1

-- COMMAND ----------

refresh table .ccu003_04_kmc_final_stroke_admissions_op

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_stroke_admissions_op

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_stroke_admissions_op_v1

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_kmc_final_stroke_admissions_op

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_kmc_final_stroke_admissions_op_v1

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_kmc_final_stroke_admissions_op

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_kmc_final_stroke_admissions_op_v1

-- COMMAND ----------

select * from .ccu003_04_kmc_final_stroke_admissions_dx_v1 where person_id = ''

-- COMMAND ----------

select count(distinct procodet) from .ccu003_04_kmc_final_stroke_admissions_op_v1
