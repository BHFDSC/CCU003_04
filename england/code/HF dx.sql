...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'hf_dx_0622'
...
-----------------------------------------------
-- HF diagnosis episodes
-----------------------------------------------
-- This query returns one row per each HF diagnosis for each episode - this means there are multiple rows per episode!!!
...
CREATE TABLE IF NOT EXISTS .ccu003_04_nh_final_admissions_${ccu003_04_spec} AS 
  ...
  GROUPED_DIAGNOSIS_DISCHARGE AS HF_DIAGNOSIS_DISCHARGE,
  GROUPED_DIAGNOSIS_DISCHARGE_POSITION AS HF_DIAGNOSIS_DISCHARGE_POSITION,
  GROUPED_DIAGNOSIS_PRIMARY AS HF_DIAGNOSIS_PRIMARY,
...
select year(admidate), count(*) from .ccu003_04_nh_final_hf_admissions_dx_old group by year(admidate) order by year(admidate)
