...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'pad_dx_2'
...
-----------------------------------------------
-- PAD diagnosis episodes
-----------------------------------------------
-- This query returns one row per each PAD diagnosis for each episode - this means there are multiple rows per episode!!!
...
LEFT(code, 3) IN ('I74','I70','I73','I77')
...
CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base1 AS
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base1   OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from 
(
select PERSON_ID, count(distinct in_spell_id) as c from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base1 group by person_id --ORDER BY PERSON_ID, count(distinct in_spell_id) desc
)
order by c desc

-- COMMAND ----------

 select * from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base1 where person_id = '' order by SpellOrder
...
FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base1 WHERE SuperSpell_SpellNo = 1
...
-- MAGIC FROM .ccu003_04_nh_{specname}_temp_superspells_base1 AS base INNER JOIN .ccu003_04_nh_{specname}_temp_superspells_lookup AS lookup ON base.Out_Spell_ID = lookup.Spell_ID AND base.PERSON_ID = lookup.Person_ID \
...
DROP TABLE IF EXISTS .ccu003_04_kmc_final_pad_admissions_dx_v2

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_pad_admissions_dx_v2 AS 
...
ALTER TABLE .ccu003_04_kmc_final_pad_admissions_dx_v2 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_kmc_final_pad_admissions_dx_v2

-- COMMAND ----------

refresh table .ccu003_04_kmc_final_pad_admissions_dx_v2

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_pad_admissions_dx_v2
...
select count(distinct SPELL_ID) from .ccu003_04_kmc_final_pad_admissions_dx_v2
...
select count(distinct PERSON_ID) from .ccu003_04_kmc_final_pad_admissions_dx_v2
...
select count(distinct procodet) from .ccu003_04_kmc_final_pad_admissions_dx_v2 
