...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'pad_op'
...
-----------------------------------------------
-- PAD operations episodes
-----------------------------------------------
-- This query returns one row per each PAD procedure for each episode - this means there are multiple rows per episode!!!
...
(LEFT(code, 3) IN ('L16', 'L48', 'L49', 'L50', 'L51', 'L52', 'L53', 'L56', 'L57', 'L58', 'L59', 'L60', 'X09', 'X10', 'X11', 'X12','L54','L63','L66') OR LEFT(code, 4) IN ('L206','L216','L652','L653','L711')) 
...
CASE
  WHEN LEFT(icd, 3) = 'I74' THEN 1
  WHEN LEFT(icd, 3) = 'I70' THEN 2
  WHEN LEFT(icd, 3) = 'I73' THEN 3
  WHEN LEFT(icd, 3) = 'I77' THEN 4
  ELSE 0
  END AS GROUPED_DIAG_epi,
CASE
  WHEN LEFT(icd, 3) = 'I74' THEN pos_icd
  WHEN LEFT(icd, 3) = 'I70' THEN pos_icd
  WHEN LEFT(icd, 3) = 'I73' THEN pos_icd
  WHEN LEFT(icd, 3) = 'I77' THEN pos_icd
  ELSE 0
  END AS GROUPED_DIAG_epi_pos,
  
--Add any other grouped diagnostic fields here  
CASE
  WHEN TRETSPEF = '192' THEN 1
ELSE 0
END AS ICU_ADMIT
...
PERSON_ID, SPELL_ID, EPIKEY,
CASE
  WHEN LEFT(op, 3) IN ('L54','L63','L66') OR LEFT(op, 4) = 'L711' THEN 2
  WHEN LEFT(op, 3) IN ('L16', 'L48', 'L49', 'L50', 'L51', 'L52', 'L53', 'L56', 'L57', 'L58', 'L59', 'L60', 'X09', 'X10', 'X11', 'X12') OR LEFT(op, 4) IN ('L206','L216','L652','L653') THEN 1 
  ELSE 0
...
COALESCE(ICU_admit.ICU_ADMIT, 0) AS ADMISSION_TO_ITU,
--Charlson
(
...
--Other grouped dx/op fields - add or remove as applicable
LEFT OUTER JOIN
(--derived columns episodes table for PAD ICU admission
...
DROP TABLE IF EXISTS .ccu003_04_kmc_final_pad_admissions_op_v1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_pad_admissions_op_v1 AS 
  ...
  --Grouped dx/op fields
  GROUPED_DIAGNOSIS_ANY AS PAD_DIAGNOSIS_ANY,
  GROUPED_DIAGNOSIS_ANY_POSITION AS PAD_DIAGNOSIS_ANY_POSITION,
  GROUPED_DIAGNOSIS_DISCHARGE AS PAD_DIAGNOSIS_DISCHARGE,
  GROUPED_DIAGNOSIS_DISCHARGE_POSITION AS PAD_DIAGNOSIS_DISCHARGE_POSITION,
  GROUPED_DIAGNOSIS_PRIMARY AS PAD_DIAGNOSIS_PRIMARY,
  PRIMARY_DISCHARGE_DIAGNOSIS_CODE,
  GROUPED_PROCEDURE AS PAD_PROCEDURE,    
...
ALTER TABLE .ccu003_04_kmc_final_pad_admissions_op_v1 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_kmc_final_pad_admissions_op_v1

-- COMMAND ----------

refresh table .ccu003_04_kmc_final_pad_admissions_op_v1

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_pad_admissions_op_v1
...
select count(distinct SPELL_ID) from .ccu003_04_kmc_final_pad_admissions_op_v1
...
select count(distinct PERSON_ID) from .ccu003_04_kmc_final_pad_admissions_op_v1
...
select count(distinct procodet) from .ccu003_04_kmc_final_pad_admissions_op_v1
