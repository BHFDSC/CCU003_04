...
--select max(disdate) from .hes_apc_ext_all_years where year(disdate) < 2098
select max(disdate) from .hes_apc_all_years where year(disdate) < 2098

-- COMMAND ----------

select max(disdate) from .ccu003_04_base_nh where year(DISDATE) < 2098
...
CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'acs_dx_0622'
...
refresh table .ccu003_04_base_nh

-- COMMAND ----------

-----------------------------------------------
-- ACS diagnosis episodes
-----------------------------------------------
-- This query returns one row per each ACS diagnosis for each episode - this means there are multiple rows per episode!!!
...
( -- need to make a CTE where the dx/op code fields are exploded into rows - then we have one row for each dx or op in each episode.
  ...
  --OPS
  --posexplode(array(opertn_01, 	opertn_02, 	opertn_03, 	opertn_04, 	opertn_05, 	opertn_06, 	opertn_07, 	opertn_08, 	opertn_09, 	opertn_10, 	opertn_11, 	opertn_12, 	opertn_13, 	opertn_14, 	opertn_15, 	opertn_16, 	opertn_17, 	opertn_18, 	opertn_19, 	opertn_20, 	opertn_21, 	opertn_22, 	opertn_23, 	opertn_24)) AS (pos_code, code)
  --DIAGS
...
(LEFT(code, 4) IN ('I240', 'I241', 'I248', 'I249', 'I200') OR LEFT(code, 3) IN ('I21','I22'))

-- COMMAND ----------

--create table if not exists .nh_acs_test_source_episodes as select * from global_temp.nh_${ccu003_04_spec}_temp_source_episodes

-- COMMAND ----------

--select count(*) FROM global_temp.nh_${ccu003_04_spec}_temp_source_episodes
...
-- MAGIC %md
-- MAGIC From the episodes with the condition, identfy the first episode per patient within the time window.<br/>
-- MAGIC INCORRECT

-- COMMAND ----------

--No longer used
--CREATE OR REPLACE GLOBAL TEMP VIEW nh_${ccu003_04_spec}_temp_first_episodes AS-- get first episode for each person where they had the spec condition in the time window
--
--SELECT distinct
...
--FROM .ccu003_04_base_nh
--WHERE epikey IN (SELECT FIRST(EPIKEY) OVER (PARTITION BY PERSON_ID ORDER BY ADMIDATE ASC, EPIORDER ASC) AS EPIKEY FROM global_temp.nh_${ccu003_04_spec}_temp_source_episodes)

-- COMMAND ----------

--SELECT *, MIN(EPIKEY) OVER (PARTITION BY PERSON_ID ORDER BY CAST(admidate AS TIMESTAMP) ASC RANGE BETWEEN INTERVAL 30 DAYS PRECEDING AND CURRENT ROW) AS MINEPIKEY FROM global_temp.nh_${ccu003_04_spec}_temp_source_episodes order by PERSON_ID, ADMIDATE, DISDATE, EPIORDER

-- COMMAND ----------

--CREATE TABLE .ccu003_04_nh_temptest_episodes_base AS
--Select * from global_temp.nh_${ccu003_04_spec}_temp_source_episodes

-- COMMAND ----------

--ALTER TABLE .ccu003_04_nh_temptest_episodes_base OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

--  --Find earliest qualifying episode in each spell 
--  SELECT * FROM
--  (
--    SELECT CASE WHEN f.MinEpiorder = Epiorder THEN 1 ELSE 0 END AS FirstInSpell, e.* FROM
--    .ccu003_04_nh_temptest_episodes_base AS e LEFT JOIN 
--    (
--      SELECT
--        PERSON_ID,
--        SPELL_ID, 
--        MIN(Epiorder) AS MinEpiorder
--      FROM .ccu003_04_nh_temptest_episodes_base
--      GROUP BY PERSON_ID, SPELL_ID
--    ) AS f ON e.PERSON_ID = f.PERSON_ID AND e.SPELL_ID = f.SPELL_ID  
--    order by e.person_ID, e.spell_ID, epiorder
--  )
--  WHERE PERSON_ID = '' --FirstInSpell = 1

-- COMMAND ----------

--CREATE OR REPLACE GLOBAL TEMP VIEW nh_${ccu003_04_spec}_temp_first_episodes_30days AS
--
----Get one record per spell, with an indicator of whether there are any other spells within 30 days
--WITH cte AS
--(
--  SELECT  
--  MIN(Admidate) OVER (PARTITION BY PERSON_ID ORDER BY CAST(Admidate AS TIMESTAMP) ASC RANGE BETWEEN INTERVAL 30 DAYS PRECEDING AND CURRENT ROW) AS MinAdmidate,
--  *
--  FROM 
--  (      
--    --Get earliest qualifying episode per spell 
--    --Returns one row per spell
--    SELECT DISTINCT    
--    Epikey,
--    PERSON_ID,
--    SPELL_ID,
...
--    (
--      SELECT CASE WHEN f.MinEpiorder = Epiorder THEN 1 ELSE 0 END AS FirstInSpell, e.* FROM
--      .ccu003_04_nh_temptest_episodes_base AS e INNER JOIN 
--      (
--        SELECT
--          PERSON_ID,
--          SPELL_ID, 
...
--      ORDER BY e.person_ID, e.spell_ID, epiorder
--    )
--    WHERE FirstInSpell = 1 and person_id = ''
--  )
--)
--
--SELECT DISTINCT
--CASE WHEN Epikey = MinEpikey THEN 1 ELSE 0 END AS IndexEpisode,
--b.Epikey,
--b.PERSON_ID,
--b.SPELL_ID,
--b.Epiorder,
...
--FROM
--(
--  select PERSON_ID, SPELL_ID, Admidate, MIN(Epikey) AS MinEpikey FROM cte GROUP BY PERSON_ID, SPELL_ID, Admidate
--) AS c
--inner join .ccu003_04_nh_temptest_episodes_base b
--ON c.PERSON_ID = b.PERSON_ID AND c.SPELL_ID = b.SPELL_ID AND c.Admidate = b.Admidate
--ORDER BY PERSON_ID, ADMIDATE, DISDATE, EPIORDER

-- COMMAND ----------

--SELECT  
--  MIN(Admidate) OVER (PARTITION BY PERSON_ID ORDER BY Admidate_Julian ASC RANGE BETWEEN 30 PRECEDING AND CURRENT ROW) AS MinAdmidate,
--  *
--  FROM 
--  (      
--    --Get earliest qualifying episode per spell 
...
--    Epistart,
--    Epiend,
--    DATEDIFF(Admidate, '2000-01-01') AS Admidate_Julian
--    FROM
--    (
--      SELECT CASE WHEN f.MinEpiorder = Epiorder THEN 1 ELSE 0 END AS FirstInSpell, e.* FROM
--      .ccu003_04_nh_temptest_episodes_base AS e INNER JOIN 
...
--    )
--    WHERE FirstInSpell = 1 and person_id = ''
--  ) order by admidate, disdate, epistart, epiend

-- COMMAND ----------

--select * from global_temp.nh_${ccu003_04_spec}_temp_first_episodes_30days where person_id in
--(
--  select person_id from
--  (
--    select PERSON_ID, count(PERSON_ID) from global_temp.nh_${ccu003_04_spec}_temp_first_episodes_30days group by PERSON_ID having Count(PERSON_ID) > 4
--  )
--)
--ORDER BY PERSON_ID, ADMIDATE, DISDATE, EPIORDER
...
--select * from global_temp.nh_${ccu003_04_spec}_temp_first_episodes_30days 
...
DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base AS
...
FROM .ccu003_04_base_nh AS base INNER JOIN 
(
...
--create table .nh_acs_test_episodes_base as select * from .nh_${ccu003_04_spec}_temp_episodes_base

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select count( distinct PROCODET) from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base 

-- COMMAND ----------

select count(*) from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base

-- COMMAND ----------

select year(admidate), count(*) from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base group by year(admidate) order by year(admidate)

-- COMMAND ----------

select year(disdate), count(*) from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base group by year(disdate) order by year(disdate)

-- COMMAND ----------

select * from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base order by PERSON_ID, SPELL_ID, EPIKEY
...
( -- CTE where the ICD fields are exploded into rows - one row for each diagnosis in each episode
  ...
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base
)
...
CASE
  WHEN LEFT(icd, 4) IN ('I210', 'I211', 'I212', 'I213') THEN 1
  WHEN LEFT(icd, 4) IN ('I214', 'I222') THEN 2
  WHEN LEFT(icd, 4) IN ('I219', 'I220', 'I221', 'I228', 'I229') THEN 3
  WHEN LEFT(icd, 4) IN ('I200') THEN 4
  WHEN LEFT(icd, 4) IN ('I248', 'I249') THEN 5
  WHEN LEFT(icd, 4) IN ('I240') THEN 6
  WHEN LEFT(icd, 4) IN ('I241') THEN 7
  ELSE 0
  END AS GROUPED_DIAG_epi,
CASE
  WHEN LEFT(icd, 4) IN ('I210', 'I211', 'I212', 'I213') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I214', 'I222') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I219', 'I220', 'I221', 'I228', 'I229') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I200') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I248', 'I249') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I240') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I241') THEN pos_icd
  ELSE 0
  END AS GROUPED_DIAG_epi_pos,
--Add any other grouped diagnostic fields here  
CASE WHEN LEFT(icd, 4) = 'I252' OR LEFT(icd, 3) = 'I50' THEN 1 ELSE 0 END AS HEART_FAILURE_epi,
CASE WHEN LEFT(icd, 4) IN ('I241', 'I442') OR LEFT(icd, 3) = 'I23' THEN 1 ELSE 0 END AS MI_COMPLICATION_epi
FROM extract_icd_exploded
...
PERSON_ID, SPELL_ID, EPIKEY,
CASE
  WHEN LEFT(op, 3) IN ('K40', 'K41', 'K42', 'K43', 'K44', 'K45', 'K46') THEN 3
  WHEN LEFT(op, 3) IN ('K49', 'K50', 'K75') THEN 2
  WHEN LEFT(op, 4) IN ('K631', 'K632', 'K633', 'K634', 'K635', 'K636') OR LEFT(op, 3) = 'K65' THEN 1 
  ELSE 0
  END AS GROUPED_PROC_epi,
--Add any other grouped procedural fields here  
CASE WHEN LEFT(op, 3) IN ('K49', 'K50', 'K75') AND op_date = admidate THEN 1 ELSE 0 END AS PRIMARY_ANGIOPLASTY_epi
...
REFRESH TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes AS
  ...
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base AS base
  --derived grouped dx
  LEFT OUTER JOIN  
  (
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes

-- COMMAND ----------

select * from .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes
...
DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base AS
  ...
  FROM
  (--base spells
    SELECT PERSON_ID, SPELL_ID, MIN(Epikey) AS AdmissionEpikey, MAX(Epikey) AS DischargeEpikey, COUNT(*) AS EpisodeCount FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes GROUP BY PERSON_ID, SPELL_ID
  ) AS base_spells
  --admission fields
  INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes AS admission_episodes ON base_spells.AdmissionEpikey = admission_episodes.Epikey
  --discharge fields
  INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes AS discharge_episodes ON base_spells.DischargeEpikey = discharge_episodes.Epikey
  ...
  (
    ...
    FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes AS a INNER JOIN
    (--first find the min hierarchical code. Once found, find the minimum position for it
      SELECT PERSON_ID, SPELL_ID, MIN(Grouped_diag) AS Grouped_diag_epi
      FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes WHERE Grouped_diag > 0 GROUP BY PERSON_ID, SPELL_ID
  ...
  (
    ...
    (--first find the min hierarchical code. Once found, find the minimum position for it
      ...
      FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes WHERE Grouped_diag > 0 GROUP BY EPIKEY
  ...
  (--find the min hierarchical code for position = 1
    SELECT EPIKEY, MIN(Grouped_diag) AS Grouped_diag_epi
    FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes WHERE Grouped_diag > 0 AND Grouped_diag_pos = 1 GROUP BY EPIKEY
  ...
  (
    ...
    FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes WHERE Grouped_proc > 0 GROUP BY PERSON_ID, SPELL_ID
  ) AS any_derived_op ON base_spells.PERSON_ID = any_derived_op.PERSON_ID AND base_spells.SPELL_ID = any_derived_op.SPELL_ID

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

refresh table .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes

-- COMMAND ----------

select * from .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base where Grouped_diag_pri = 0

-- COMMAND ----------

select * from .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes where spell_id = 5068286873

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells
...
CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells AS
...
COALESCE(Heart_failure.Heart_failure_epi, 0) AS PRESENCE_OF_HEART_FAILURE,
COALESCE(MI_complication.MI_complication_epi, 0) AS COMPLICATION_OF_ACUTE_MI,
COALESCE(Primary_angioplasty.Primary_angioplasty_epi, 0) AS POSS_PRIM_ANGIOPLASTY,
--Charlson
(
  ...
  --Charlson.CHARLSON_MYOCARD_INFARCT*1+ --removed for this spec
...
FROM .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base AS base 
...
(--base episodes table for died
  ...
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes WHERE died > 0 GROUP BY PERSON_ID, SPELL_ID
) AS Died ON base.PERSON_ID = Died.PERSON_ID AND base.SPELL_ID = Died.SPELL_ID

--Other grouped dx/op fields - add or remove as applicable
LEFT OUTER JOIN
(--derived columns episodes table for heart failure
  SELECT PERSON_ID, SPELL_ID, MAX(HEART_FAILURE_epi) AS Heart_failure_epi
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_diagcols WHERE HEART_FAILURE_epi > 0 GROUP BY PERSON_ID, SPELL_ID
) AS Heart_failure ON base.PERSON_ID = Heart_failure.PERSON_ID AND base.SPELL_ID = Heart_failure.SPELL_ID
LEFT OUTER JOIN
(--derived columns episodes table for MI complication
  SELECT PERSON_ID, SPELL_ID, MAX(MI_COMPLICATION_epi) AS MI_complication_epi
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_diagcols WHERE MI_COMPLICATION_epi > 0 GROUP BY PERSON_ID, SPELL_ID
) AS MI_complication ON base.PERSON_ID = MI_complication.PERSON_ID AND base.SPELL_ID = MI_complication.SPELL_ID
LEFT OUTER JOIN
(--derived columns episodes table for angioplasty
  SELECT PERSON_ID, SPELL_ID, MAX(PRIMARY_ANGIOPLASTY_epi) AS Primary_angioplasty_epi
  FROM global_temp.nh_${ccu003_04_spec}_temp_episodes_opercols WHERE PRIMARY_ANGIOPLASTY_epi > 0 GROUP BY PERSON_ID, SPELL_ID
) AS Primary_angioplasty ON base.PERSON_ID = Primary_angioplasty.PERSON_ID AND base.SPELL_ID = Primary_angioplasty.SPELL_ID
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells

-- COMMAND ----------

select * from .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells where GROUPED_DIAGNOSIS_PRIMARY > 0
...
WITH cte AS
(
  SELECT PERSON_ID, SPELL_ID, Admidate, Disdate, Disdest, Dismeth, SPELL_ID, Admidate, Disdate, Admimeth, Dismeth, PRIMARY_DISCHARGE_DIAGNOSIS_CODE, ROW_NUMBER() OVER (PARTITION BY PERSON_ID ORDER BY Admidate, Disdate, SPELL_ID) AS RowNo FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells WHERE SPELL_ID IS NOT NULL
)
...
REFRESH TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base
...
CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base AS
...
--where former.person_id = ''
--ORDER BY former.PERSON_ID, former.SpellOrder

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base   OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select * from 
(
select PERSON_ID, count(distinct in_spell_id) as c from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base group by person_id --ORDER BY PERSON_ID, count(distinct in_spell_id) desc
)
order by c desc

-- COMMAND ----------

 select * from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base where person_id = '' order by SpellOrder
...
DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS
...
FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base WHERE SuperSpell_SpellNo = 1

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup OWNER TO `nick.hall@dph.ox.ac.uk`
...
-- MAGIC 
-- MAGIC #df1 = spark.sql(f"SELECT * FROM .ccu003_04_nh_acs_superspells_base")
-- MAGIC #print(df1.count())
-- MAGIC 
-- MAGIC #df2 = spark.sql(f"SELECT * FROM .ccu003_04_nh_acs_superspells_lookup")
...
-- MAGIC FROM .ccu003_04_nh_{specname}_temp_superspells_base AS base INNER JOIN .ccu003_04_nh_{specname}_temp_superspells_lookup AS lookup ON base.Out_Spell_ID = lookup.Spell_ID AND base.PERSON_ID = lookup.Person_ID \
-- MAGIC WHERE base.In_Spell_ID NOT IN (SELECT Spell_ID FROM .ccu003_04_nh_{specname}_temp_superspells_lookup WHERE Person_ID = lookup.PERSON_ID AND Superspell_ID = lookup.Superspell_ID)"
-- MAGIC 
-- MAGIC rowcount = 1
-- MAGIC loopcount = 1
-- MAGIC 
...
-- MAGIC   df.write.insertInto(f".ccu003_04_nh_{specname}_temp_superspells_lookup", overwrite = False)
...
-- MAGIC #FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base AS base INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS lookup ON base.Out_Spell_ID = lookup.Spell_ID AND base.PERSON_ID = lookup.Person_ID
-- MAGIC #WHERE base.In_Spell_ID NOT IN (SELECT Spell_ID FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup WHERE Person_ID = lookup.PERSON_ID AND Superspell_ID = lookup.Superspell_ID)"""
-- MAGIC 
-- MAGIC #rowcount = 1
-- MAGIC #loopcount = 1
-- MAGIC 
...
-- MAGIC #  df.write.insertInto('.ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup', overwrite = False)

-- COMMAND ----------

SELECT * FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup ORDER BY Person_ID, Superspell_ID, SpellNo

-- COMMAND ----------

select person_ID, superspell_ID, count(superspell_ID) from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup group by person_ID, superspell_ID order by count(superspell_ID) desc, person_id asc

-- COMMAND ----------

select * from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base where person_id = '' order by epistart, epiorder

-- COMMAND ----------

select spellcount, count(spellcount) from
(select person_ID, superspell_ID, count(superspell_ID) as spellcount from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup group by person_ID, superspell_ID)
group by spellcount
...
SELECT base.Person_ID, base.Superspell_ID, spells.Spell_ID, spells.Procodet, spells.Admidate, spells.Admiage, spells.Sex, Spells.Ethnos, Spells.Resgor_ons, spells.Admimeth FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS base
INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells spells on base.Person_ID = spells.Person_ID and base.Spell_ID = spells.Spell_ID
...
SELECT base.* FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS base
INNER JOIN
(
  SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup GROUP BY Person_ID, Superspell_ID
) AS dis
...
(  
  ...
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells AS spells --to provide dx info
  INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS base --to provide superspell info
...
select * from .ccu003_04_nh_${ccu003_04_spec}_temp_final_episodes where person_id = '' order by spell_id, epiorder
...
REFRESH TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup
...
DROP TABLE IF EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_final_superspells 

-- COMMAND ----------

--select count(distinct procodet) from .ccu003_04_nh_final_acs_admissions_dx

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_${ccu003_04_spec}_temp_final_superspells AS
...
PRESENCE_OF_HEART_FAILURE,
COMPLICATION_OF_ACUTE_MI,
POSS_PRIM_ANGIOPLASTY,
--Charlson
(
  CHARLSON_CANCER*1+
  CHARLSON_CEREBRO*1+
...
(--Admission spell
  ...
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS base
  INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells AS spells
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
  WHERE base.SpellNo = 1
) AS admi
INNER JOIN
(--Discharge spell
  SELECT base.Disdate, base.Dismeth, base.Primary_dx, base.Person_ID, base.SuperSpell_ID FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS base --these items have been carried through into the lookup for ease of access
  INNER JOIN
  (
    SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup GROUP BY Person_ID, Superspell_ID --assumes SpellNo is applied in chronological order
  ) AS dis
  ON base.Person_ID = dis.Person_ID AND base.Superspell_ID = dis.Spell_ID AND base.SpellNo = dis.LastSpellNo
) AS dis
...
(--Derived columns, except grouped dx/op
  SELECT 
    ...
    --Additional grouped dx/op fields - add or remove as applicable
    MAX(PRESENCE_OF_HEART_FAILURE) AS PRESENCE_OF_HEART_FAILURE,
    MAX(COMPLICATION_OF_ACUTE_MI) AS COMPLICATION_OF_ACUTE_MI,    
    MAX(POSS_PRIM_ANGIOPLASTY) AS POSS_PRIM_ANGIOPLASTY,
    --Charlson
    MAX(CHARLSON_CANCER) AS CHARLSON_CANCER,
    MAX(CHARLSON_CEREBRO) AS CHARLSON_CEREBRO,
    MAX(CHARLSON_CPD) AS CHARLSON_CPD,
    ...
    MAX(CHARLSON_HIV) AS CHARLSON_HIV,
    MAX(CHARLSON_MILD_LIVER) AS CHARLSON_MILD_LIVER
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells AS spells
  INNER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS base
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
...
ALTER TABLE .ccu003_04_nh_${ccu003_04_spec}_temp_final_superspells OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_04_nh_${ccu003_04_spec}_temp_final_superspells
...
DROP TABLE IF EXISTS .ccu003_04_nh_final_admissions_${ccu003_04_spec}

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_final_admissions_${ccu003_04_spec} AS 
  ...
  --Grouped dx/op fields
  GROUPED_DIAGNOSIS_ANY AS IHD_DIAGNOSIS_ANY,
  GROUPED_DIAGNOSIS_ANY_POSITION AS IHD_DIAGNOSIS_ANY_POSITION,
  GROUPED_DIAGNOSIS_DISCHARGE AS IHD_DIAGNOSIS_DISCHARGE,
  GROUPED_DIAGNOSIS_DISCHARGE_POSITION AS IHD_DIAGNOSIS_DISCHARGE_POSITION,
  GROUPED_DIAGNOSIS_PRIMARY AS IHD_DIAGNOSIS_PRIMARY,
  PRIMARY_DISCHARGE_DIAGNOSIS_CODE,
  GROUPED_PROCEDURE AS CORONARY_PROCEDURE,  
  ...
  CHARLSON_HIV,
  CHARLSON_MILD_LIVER
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_spells AS base
  FULL OUTER JOIN .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_lookup AS lookup ON base.Person_ID = lookup.Person_ID AND base.Spell_ID = lookup.Spell_ID
  ...
  CHARLSON_HIV,
  CHARLSON_MILD_LIVER
  FROM .ccu003_04_nh_${ccu003_04_spec}_temp_final_superspells 
...
ALTER TABLE .ccu003_04_nh_final_admissions_${ccu003_04_spec} OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_nh_final_admissions_${ccu003_04_spec}

-- COMMAND ----------

refresh table .ccu003_04_nh_final_acs_admissions_dx_old

-- COMMAND ----------

select count(*) from .ccu003_04_nh_final_acs_admissions_dx_old

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_nh_final_acs_admissions_dx_old

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_nh_final_acs_admissions_dx_old

-- COMMAND ----------

select * from .ccu003_04_nh_final_admissions_${ccu003_04_spec} where person_id = ''

-- COMMAND ----------

select count(distinct procodet) from .ccu003_04_nh_final_admissions_${ccu003_04_spec}

-- COMMAND ----------

select year(admidate), count(*) from .ccu003_04_nh_final_admissions_${ccu003_04_spec} group by year(admidate) order by year(admidate)

-- COMMAND ----------

select year(admidate), count(*) from .ccu003_04_nh_final_acs_admissions_dx_old group by year(admidate) order by year(admidate)
