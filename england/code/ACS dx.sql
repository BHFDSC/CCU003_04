...
create table if not exists .nh_acs_test_source_episodes as select * from global_temp.nh_temp_source_episodes
...
create table .nh_acs_test_first_episodes as select * from global_temp.nh_temp_first_episodes

-- COMMAND ----------

select year(admidate), count(*) from global_temp.nh_temp_first_episodes group by year(admidate) order by year(admidate)

-- COMMAND ----------

select year(disdate), count(*) from global_temp.nh_temp_first_episodes group by year(disdate) order by year(disdate)
...
create table .nh_acs_test_episodes_base as select * from .nh_temp_episodes_base

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_temp_episodes_base OWNER TO `nick.hall@dph.ox.ac.uk`
...
select year(admidate), count(*) from .ccu003_04_nh_temp_episodes_base group by year(admidate) order by year(admidate)

-- COMMAND ----------

select year(disdate), count(*) from .ccu003_04_nh_temp_episodes_base group by year(disdate) order by year(disdate)
...
ALTER TABLE .ccu003_04_nh_temp_final_episodes1 OWNER TO `nick.hall@dph.ox.ac.uk`
...
ALTER TABLE .ccu003_04_nh_temp_spells_base9 OWNER TO `nick.hall@dph.ox.ac.uk`
...
select * from .ccu003_04_nh_temp_final_episodes1 where spell_id = 5068286873
...
(--derived columns episodes table for heart failure
  ...
  FROM global_temp.nh_temp_episodes_diagcols WHERE HEART_FAILURE_epi > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for MI complication
  ...
  FROM global_temp.nh_temp_episodes_diagcols WHERE MI_COMPLICATION_epi > 0 GROUP BY PERSON_ID, SPELL_ID
...
(--derived columns episodes table for angioplasty
  ...
  FROM global_temp.nh_temp_episodes_opercols WHERE PRIMARY_ANGIOPLASTY_epi > 0 GROUP BY PERSON_ID, SPELL_ID
...
ALTER TABLE .ccu003_04_nh_temp_final_spells OWNER TO `nick.hall@dph.ox.ac.uk`
...
DROP TABLE IF EXISTS .ccu003_04_nh_temp_superspells_base

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_superspells_base AS

SELECT *, ROW_NUMBER() OVER (PARTITION BY PERSON_ID ORDER BY Out_RowNo) AS SpellOrder FROM global_temp.nh_temp_superspells_base

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_temp_superspells_base OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_nh_temp_superspells_base ORDER BY PERSON_ID, SpellOrder
...
DROP TABLE IF EXISTS .ccu003_04_nh_temp_superspells_lookup

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_temp_superspells_lookup AS
...
FROM .ccu003_04_nh_temp_superspells_base WHERE SpellOrder = 1

-- COMMAND ----------

ALTER TABLE .ccu003_04_nh_temp_superspells_lookup OWNER TO `nick.hall@dph.ox.ac.uk`
...
-- MAGIC FROM .ccu003_04_nh_temp_superspells_base AS base INNER JOIN .ccu003_04_nh_temp_superspells_lookup AS lookup ON base.Out_Spell_ID = lookup.Spell_ID AND base.PERSON_ID = lookup.Person_ID
-- MAGIC WHERE base.In_Spell_ID NOT IN (SELECT Spell_ID FROM .ccu003_04_nh_temp_superspells_lookup WHERE Person_ID = lookup.PERSON_ID AND Superspell_ID = lookup.Superspell_ID)"""
-- MAGIC 
-- MAGIC rowcount = 1
-- MAGIC loopcount = 1
-- MAGIC 
...
-- MAGIC   df.write.insertInto('.ccu003_04_nh_temp_superspells_lookup', overwrite = False)

-- COMMAND ----------

SELECT * FROM .ccu003_04_nh_temp_superspells_lookup ORDER BY Person_ID, Superspell_ID, SpellNo

-- COMMAND ----------

select person_ID, superspell_ID, count(superspell_ID) from .ccu003_04_nh_temp_superspells_lookup group by person_ID, superspell_ID order by count(superspell_ID) desc, person_id asc

-- COMMAND ----------

select * from .ccu003_04_nh_temp_episodes_base where person_id = ''

-- COMMAND ----------

select spellcount, count(spellcount) from
(select person_ID, superspell_ID, count(superspell_ID) as spellcount from .ccu003_04_nh_temp_superspells_lookup group by person_ID, superspell_ID)
group by spellcount
...
SELECT base.Person_ID, base.Superspell_ID, spells.Spell_ID, spells.Procodet, spells.Admidate, spells.Admiage, spells.Sex, Spells.Ethnos, Spells.Resgor_ons, spells.Admimeth FROM .ccu003_04_nh_temp_superspells_lookup AS base
...
SELECT base.* FROM .ccu003_04_nh_temp_superspells_lookup AS base
INNER JOIN
(
  SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_04_nh_temp_superspells_lookup GROUP BY Person_ID, Superspell_ID
) AS dis
...
(  
  ...
  INNER JOIN .ccu003_04_nh_temp_superspells_lookup AS base --to provide superspell info
...
select * from .ccu003_04_nh_temp_final_episodes1 where person_id = '' order by spell_id, epiorder
...
REFRESH TABLE .ccu003_04_nh_temp_superspells_lookup
...
(--Admission spell
  ...
  FROM .ccu003_04_nh_temp_superspells_lookup AS base
...
INNER JOIN
(--Discharge spell
  SELECT base.Disdate, base.Dismeth, base.Primary_dx, base.Person_ID, base.SuperSpell_ID FROM .ccu003_04_nh_temp_superspells_lookup AS base --these items have been carried through into the lookup for ease of access
  INNER JOIN
  (
    SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_04_nh_temp_superspells_lookup GROUP BY Person_ID, Superspell_ID --assumes SpellNo is applied in chronological order
  ) AS dis
  ON base.Person_ID = dis.Person_ID AND base.Superspell_ID = dis.Spell_ID AND base.SpellNo = dis.LastSpellNo
) AS dis
...
(--Derived columns, except grouped dx/op
  ...
  INNER JOIN .ccu003_04_nh_temp_superspells_lookup AS base
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
...
ALTER TABLE .ccu003_04_nh_temp_final_superspells OWNER TO `nick.hall@dph.ox.ac.uk`
...
DROP TABLE IF EXISTS .ccu003_04_nh_final_acs_admissions_dx

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_final_acs_admissions_dx AS 
  ...
  FULL OUTER JOIN .ccu003_04_nh_temp_superspells_lookup AS lookup ON base.Person_ID = lookup.Person_ID AND base.Spell_ID = lookup.Spell_ID
...
ALTER TABLE .ccu003_04_nh_final_acs_admissions_dx OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_nh_final_acs_admissions_dx

-- COMMAND ----------

select count(*) from .ccu003_04_nh_final_acs_admissions_dx

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_nh_final_acs_admissions_dx

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_nh_final_acs_admissions_dx

-- COMMAND ----------

select * from .ccu003_04_nh_final_acs_admissions_dx where person_id = ''
