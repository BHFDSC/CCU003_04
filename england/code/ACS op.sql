...
select max(disdate) from .ccu003_04_base_nh
...
REMOVE WIDGET ccu003_04_spec

-- COMMAND ----------

CREATE WIDGET TEXT ccu003_04_spec DEFAULT 'acs_op_0622'
...
-----------------------------------------------
-- ACS procedure episodes
-----------------------------------------------
-- This query returns one row per each ACS procedure for each episode - this means there are multiple rows per episode!!!
...
(LEFT(code, 4) IN ('K631', 'K632', 'K633', 'K634', 'K635', 'K636') OR LEFT(code, 3) IN ('K40', 'K41', 'K42', 'K43', 'K44', 'K45', 'K46', 'K49', 'K50', 'K65', 'K75'))
...
select count(distinct epikey) from global_temp.nh_${ccu003_04_spec}_temp_source_episodes

-- COMMAND ----------

--SELECT count(*) FROM global_temp.nh_${ccu003_04_spec}_temp_first_episodes
...
select year(admidate), count(distinct PERSON_ID) from .ccu003_04_nh_${ccu003_04_spec}_temp_episodes_base group by year(admidate) order by year(admidate)
...
refresh table .ccu003_04_nh_${ccu003_04_spec}_temp_spells_base
...
select * from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base ORDER BY PERSON_ID, superspellno
...
select * from .ccu003_04_nh_${ccu003_04_spec}_temp_superspells_base where person_id = '' order by superspellno
...
refresh table .ccu003_04_nh_final_admissions_${ccu003_04_spec}
...
select year(admidate), count(*) from .ccu003_04_nh_final_acs_admissions_op_old group by year(admidate) order by year(admidate)

-- COMMAND ----------

select * from .ccu003_04_nh_final_acs_admissions_op_old where spell_id not in (select spell_id from .ccu003_04_nh_final_admissions_${ccu003_04_spec})

-- COMMAND ----------

select count(*) from .ccu003_04_nh_final_admissions_${ccu003_04_spec}

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_nh_final_admissions_${ccu003_04_spec}
