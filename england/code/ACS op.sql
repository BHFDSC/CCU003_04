...
select max(disdate) from .ccu003_04_nh_base
...
select count(distinct epikey) from global_temp.nh_temp_source_episodes

-- COMMAND ----------

SELECT count(*) FROM global_temp.nh_temp_first_episodes
...
select year(admidate), count(distinct PERSON_ID) from .ccu003_04_nh_temp_episodes_base group by year(admidate) order by year(admidate)
...
refresh table .ccu003_04_nh_temp_spells_base9
...
DROP TABLE IF EXISTS .ccu003_04_nh_final_acs_admissions_op

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_nh_final_acs_admissions_op AS 
...
ALTER TABLE .ccu003_04_nh_final_acs_admissions_op OWNER TO `nick.hall@dph.ox.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_nh_final_acs_admissions_op

-- COMMAND ----------

select year(admidate), count(*) from .ccu003_04_nh_temp_final_episodes1 group by year(admidate) order by year(admidate)

-- COMMAND ----------

select count(*) from .ccu003_04_nh_final_acs_admissions_op

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_nh_final_acs_admissions_op
