...
DROP TABLE IF EXISTS .ccu003_nh_temp_episodes_base

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_episodes_base AS
...
ALTER TABLE .ccu003_nh_temp_episodes_base OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_nh_temp_episodes_base

-- COMMAND ----------

select * from .ccu003_nh_temp_episodes_base order by PERSON_ID, SPELL_ID, EPIKEY
...
( -- CTE where the ICD fields are exploded into rows - one row for each diagnosis in each episode
  ...
  FROM .ccu003_nh_temp_episodes_base
)
...
REFRESH TABLE .ccu003_nh_temp_episodes_base

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_nh_temp_final_episodes1

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_final_episodes1 AS
  ...
  FROM .ccu003_nh_temp_episodes_base AS base
  --derived grouped dx
  LEFT OUTER JOIN  
  (
...
ALTER TABLE .ccu003_nh_temp_final_episodes1 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select count(*) from .ccu003_nh_temp_final_episodes1
...
DROP TABLE IF EXISTS .ccu003_nh_temp_spells_base9

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_spells_base9 AS
  ...
  FROM
  (--base spells
    SELECT PERSON_ID, SPELL_ID, MIN(Epikey) AS AdmissionEpikey, MAX(Epikey) AS DischargeEpikey, COUNT(*) AS EpisodeCount FROM .ccu003_nh_temp_final_episodes1 GROUP BY PERSON_ID, SPELL_ID
  ) AS base_spells
  --admission fields
  INNER JOIN .ccu003_nh_temp_final_episodes1 AS admission_episodes ON base_spells.AdmissionEpikey = admission_episodes.Epikey
  --discharge fields
  INNER JOIN .ccu003_nh_temp_final_episodes1 AS discharge_episodes ON base_spells.DischargeEpikey = discharge_episodes.Epikey
  ...
  (
    ...
    FROM .ccu003_nh_temp_final_episodes1 AS a INNER JOIN
    (--first find the min hierarchical code. Once found, find the minimum position for it
      SELECT PERSON_ID, SPELL_ID, MIN(Grouped_diag) AS Grouped_diag_epi
      FROM .ccu003_nh_temp_final_episodes1 WHERE Grouped_diag > 0 GROUP BY PERSON_ID, SPELL_ID
  ...
  (
    ...
    (--first find the min hierarchical code. Once found, find the minimum position for it
      ...
      FROM .ccu003_nh_temp_final_episodes1 WHERE Grouped_diag > 0 GROUP BY EPIKEY
  ...
  (--find the min hierarchical code for position = 1
    SELECT EPIKEY, MIN(Grouped_diag) AS Grouped_diag_epi
    FROM .ccu003_nh_temp_final_episodes1 WHERE Grouped_diag > 0 AND Grouped_diag_pos = 1 GROUP BY EPIKEY
  ...
  (
    ...
    FROM .ccu003_nh_temp_final_episodes1 WHERE Grouped_proc > 0 GROUP BY PERSON_ID, SPELL_ID
  ) AS any_derived_op ON base_spells.PERSON_ID = any_derived_op.PERSON_ID AND base_spells.SPELL_ID = any_derived_op.SPELL_ID

-- COMMAND ----------

ALTER TABLE .ccu003_nh_temp_spells_base9 OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

refresh table .ccu003_nh_temp_final_episodes1

-- COMMAND ----------

refresh table .ccu003_nh_temp_spells_base9

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_nh_temp_final_spells1
...
CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_final_spells1 AS
...
FROM .ccu003_nh_temp_spells_base9 AS base 
...
(--base episodes table for died
  ...
  FROM .ccu003_nh_temp_final_episodes1 WHERE died > 0 GROUP BY PERSON_ID, SPELL_ID
...
ALTER TABLE .ccu003_nh_temp_final_spells1 OWNER TO `rmhikmc@ucl.ac.uk`
...
WITH cte AS
(SELECT PERSON_ID, SPELL_ID, Admidate, Disdate, Disdest, Dismeth, SPELL_ID, Admidate, Disdate, Admimeth, Dismeth, PRIMARY_DISCHARGE_DIAGNOSIS_CODE, ROW_NUMBER() OVER (PARTITION BY PERSON_ID ORDER BY Admidate, Disdate, SPELL_ID) AS RowNo FROM .ccu003_nh_temp_final_spells1 WHERE SPELL_ID IS NOT NULL)
...
REFRESH TABLE .ccu003_nh_temp_final_spells1

-- COMMAND ----------

DROP TABLE IF EXISTS .ccu003_nh_temp_superspells_base

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_superspells_base AS

SELECT *, ROW_NUMBER() OVER (PARTITION BY PERSON_ID ORDER BY Out_RowNo) AS SpellOrder FROM global_temp.nh_temp_superspells_base

-- COMMAND ----------

ALTER TABLE .ccu003_nh_temp_superspells_base OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_nh_temp_superspells_base ORDER BY PERSON_ID, SpellOrder
...
DROP TABLE IF EXISTS .ccu003_nh_temp_superspells_lookup

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_superspells_lookup AS
...
FROM .ccu003_nh_temp_superspells_base WHERE SpellOrder = 1

-- COMMAND ----------

ALTER TABLE .ccu003_nh_temp_superspells_lookup OWNER TO `rmhikmc@ucl.ac.uk`
...
-- MAGIC %py
-- MAGIC 
-- MAGIC #df1 = spark.sql(f"SELECT * FROM .ccu003_nh_acs_superspells_base")
-- MAGIC #print(df1.count())
-- MAGIC 
-- MAGIC #df2 = spark.sql(f"SELECT * FROM .ccu003_nh_acs_superspells_lookup")
...
-- MAGIC FROM .ccu003_nh_temp_superspells_base AS base INNER JOIN .ccu003_nh_temp_superspells_lookup AS lookup ON base.Out_Spell_ID = lookup.Spell_ID AND base.PERSON_ID = lookup.Person_ID
-- MAGIC WHERE base.In_Spell_ID NOT IN (SELECT Spell_ID FROM .ccu003_nh_temp_superspells_lookup WHERE Person_ID = lookup.PERSON_ID AND Superspell_ID = lookup.Superspell_ID)"""
-- MAGIC 
-- MAGIC rowcount = 1
-- MAGIC loopcount = 1
-- MAGIC 
...
-- MAGIC   df.write.insertInto('.ccu003_nh_temp_superspells_lookup', overwrite = False)

-- COMMAND ----------

SELECT * FROM .ccu003_nh_temp_superspells_lookup ORDER BY Person_ID, Superspell_ID, SpellNo

-- COMMAND ----------

select person_ID, superspell_ID, count(superspell_ID) from .ccu003_nh_temp_superspells_lookup group by person_ID, superspell_ID order by count(superspell_ID) desc, person_id asc

-- COMMAND ----------

select spellcount, count(spellcount) from
(select person_ID, superspell_ID, count(superspell_ID) as spellcount from .ccu003_nh_temp_superspells_lookup group by person_ID, superspell_ID)
group by spellcount
...
SELECT base.Person_ID, base.Superspell_ID, spells.Spell_ID, spells.Procodet, spells.Admidate, spells.Admiage, spells.Sex, Spells.Ethnos, Spells.Resgor_ons, spells.Admimeth FROM .ccu003_nh_temp_superspells_lookup AS base
INNER JOIN .ccu003_nh_temp_final_spells1 spells on base.Person_ID = spells.Person_ID and base.Spell_ID = spells.Spell_ID
...
SELECT base.* FROM .ccu003_nh_temp_superspells_lookup AS base
INNER JOIN
(
  SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_nh_temp_superspells_lookup GROUP BY Person_ID, Superspell_ID
) AS dis
...
(  
  ...
  FROM .ccu003_nh_temp_final_spells1 AS spells --to provide dx info
  INNER JOIN .ccu003_nh_temp_superspells_lookup AS base --to provide superspell info
...
select * from .ccu003_nh_temp_final_episodes1 where person_id = '' order by spell_id, epiorder
...
REFRESH TABLE .ccu003_nh_temp_superspells_lookup
...
DROP TABLE IF EXISTS .ccu003_nh_temp_final_superspells 

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_nh_temp_final_superspells AS
...
(--Admission spell
  ...
  FROM .ccu003_nh_temp_superspells_lookup AS base
  INNER JOIN .ccu003_nh_temp_final_spells1 AS spells
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
  WHERE base.SpellNo = 1
) AS admi
INNER JOIN
(--Discharge spell
  SELECT base.Disdate, base.Dismeth, base.Primary_dx, base.Person_ID, base.SuperSpell_ID FROM .ccu003_nh_temp_superspells_lookup AS base --these items have been carried through into the lookup for ease of access
  INNER JOIN
  (
    SELECT Person_ID, Superspell_ID AS Spell_ID, MAX(SpellNo) AS LastSpellNo FROM .ccu003_nh_temp_superspells_lookup GROUP BY Person_ID, Superspell_ID --assumes SpellNo is applied in chronological order
  ) AS dis
  ON base.Person_ID = dis.Person_ID AND base.Superspell_ID = dis.Spell_ID AND base.SpellNo = dis.LastSpellNo
) AS dis
...
(--Derived columns, except grouped dx/op
  SELECT 
    ...
    MAX(CHARLSON_HIV) AS CHARLSON_HIV,
    MAX(CHARLSON_MILD_LIVER) AS CHARLSON_MILD_LIVER
  FROM .ccu003_nh_temp_final_spells1 AS spells
  INNER JOIN .ccu003_nh_temp_superspells_lookup AS base
  ON base.Person_ID = spells.Person_ID AND base.Spell_ID = spells.Spell_ID
...
ALTER TABLE .ccu003_nh_temp_final_superspells OWNER TO `rmhikmc@ucl.ac.uk`
...
DROP TABLE IF EXISTS .ccu003_kmc_final_vte_admissions_dx

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_vte_admissions_dx AS 
  ...
  CHARLSON_HIV,
  CHARLSON_MILD_LIVER
  FROM .ccu003_nh_temp_final_spells1 AS base
  FULL OUTER JOIN .ccu003_nh_temp_superspells_lookup AS lookup ON base.Person_ID = lookup.Person_ID AND base.Spell_ID = lookup.Spell_ID
  ...
  CHARLSON_HIV,
  CHARLSON_MILD_LIVER
  FROM .ccu003_nh_temp_final_superspells 
...
ALTER TABLE .ccu003_04_kmc_final_vte_admissions_dx OWNER TO `rmhikmc@ucl.ac.uk`

-- COMMAND ----------

select * from .ccu003_04_kmc_final_vte_admissions_dx

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_vte_admissions_dx
