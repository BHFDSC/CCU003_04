...
CASE
  WHEN LEFT(icd, 4) IN ('I710', 'I711', 'I713', 'I715','I718') THEN 1
  WHEN LEFT(icd, 4) IN ('I712', 'I714','I716','I719') THEN 2
  ELSE 0
  END AS GROUPED_DIAG_epi,
CASE
  WHEN LEFT(icd, 4) IN ('I710', 'I711', 'I713', 'I715','I718') THEN pos_icd
  WHEN LEFT(icd, 4) IN ('I712', 'I714','I716','I719') THEN pos_icd
  ELSE 0
  END AS GROUPED_DIAG_epi_pos,
  
--Add any other grouped diagnostic fields here  
CASE 
  WHEN LEFT(icd, 4) IN ('I710', 'I718') THEN 1 
  WHEN LEFT(icd, 4) = 'I711' THEN 2
  WHEN LEFT(icd, 4) = 'I712' THEN 3
  WHEN LEFT(icd, 4) = 'I713' THEN 4
  WHEN LEFT(icd, 4) = 'I714' THEN 5
  WHEN LEFT(icd, 4) = 'I715' THEN 6
  WHEN LEFT(icd, 4) = 'I716' THEN 7
  WHEN LEFT(icd, 4) = 'I719' THEN 8
ELSE 0 
END AS AA_TYPE_LOC_epi,
...
PERSON_ID, SPELL_ID, EPIKEY,
CASE
  WHEN LEFT(op, 3) IN ('L18', 'L19', 'L20', 'L21', 'L22', 'L23','L25') THEN 2
  WHEN LEFT(op, 3) IN ('L26', 'L27', 'L28') THEN 1 
  ELSE 0
...
COALESCE(AA_type_loc.AA_TYPE_LOC, 0) AS AA_DIAGNOSIS_TYPE_LOCATION,
...
DROP TABLE IF EXISTS .ccu003_04_kmc_final_AA_admissions_dx_v4

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS .ccu003_04_kmc_final_AA_admissions_dx_v5 AS 
  ...
  --Grouped dx/op fields
  GROUPED_DIAGNOSIS_ANY AS AA_DIAGNOSIS_ANY,
  GROUPED_DIAGNOSIS_ANY_POSITION AS AA_DIAGNOSIS_ANY_POSITION,
  GROUPED_DIAGNOSIS_DISCHARGE AS AA_DIAGNOSIS_DISCHARGE,
  GROUPED_DIAGNOSIS_DISCHARGE_POSITION AS AA_DIAGNOSIS_DISCHARGE_POSITION,
...
select * from .ccu003_04_kmc_final_AA_admissions_dx_v4

-- COMMAND ----------

refresh table .ccu003_04_kmc_final_AA_admissions_dx_v5

-- COMMAND ----------

select count(*) from .ccu003_04_kmc_final_AA_admissions_dx_v5

-- COMMAND ----------

select count(distinct SPELL_ID) from .ccu003_04_kmc_final_AA_admissions_dx_v5

-- COMMAND ----------

select count(distinct PERSON_ID) from .ccu003_04_kmc_final_AA_admissions_dx_v5

-- COMMAND ----------

select * from .ccu003_04_kmc_final_AA_admissions_dx_v5 where person_id = ''

-- COMMAND ----------

select count(distinct procodet) from .ccu003_04_kmc_final_AA_admissions_dx_v5
