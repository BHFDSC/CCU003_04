...
condition_prefix="IHD_dx"
dataspec_prefix="acute IHD dx"
top_dir="../.."
...
dat$hier_1=0
dat[dat$DIAG_CD %in% c("I210", "I211", "I212", "I213"),]$"hier_1"=1
dat[dat$DIAG_CD %in% c("I214", "I222"),]$"hier_1"=2
dat[dat$DIAG_CD %in% c("I219", "I220", "I221", "I228","I229"),]$"hier_1"=3
dat[dat$DIAG_CD %in% c("I200"),]$"hier_1"=4
dat[dat$DIAG_CD %in% c("I248", "I249"),]$"hier_1"=5
dat[dat$DIAG_CD %in% c("I240"),]$"hier_1"=6
dat[dat$DIAG_CD %in% c("I241"),]$"hier_1"=7

#Group data by episode and get highest hierarchy for that whole episode
dat = dat %>% 
      group_by(SPELL_NUM_E,PROV_UNIT_CD,EPI_NUM) %>% 
      mutate(episode_hier_1=min(hier_1[hier_1>0])) #Get hierarchies over all episodes - at diagnosis level  
...
#This should be true - otherwise some conditions are missing. Uncomment line below to see which 
...
#These Infs are caused by spells with episodes that lack any relevant diagnosis/procedure. 
#So let's just cut those out

print(paste0("REMOVING ", length(dat_spell[dat_spell$hier1==Inf,]) ," INF RECORDS"))
print(paste0("OF ", length(unique(dat_spell[dat_spell$hier1==Inf,]$SPELL_NUM_E)) ," SPELL(S)"))
