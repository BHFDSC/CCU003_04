...
condition_prefix="HF_op"
dataspec_prefix="Heart failure procedures"
top_dir="../.."
...
codes=c("K60","K61","K59","K541","K542","K02","K01")
names(codes)=c(rep("HF_pacemaker",3),rep("HF_vad",4))
...
dat$hier_1=0
dat[dat$OPER_CD_123 %in% c("K60", "K61"),]$"hier_1"=1
dat[dat$OPER_CD_123 %in% c("K59"),]$"hier_1"=2
dat[dat$OPER_CD %in% c("K541", "K542"),]$"hier_1"=3
dat[dat$OPER_CD_123 %in% c("K02"),]$"hier_1"=4
dat[dat$OPER_CD_123 %in% c("K01"),]$"hier_1"=5
...
dat_spell = 
  ...
  summarize(patient_number=first(PAT_ID_E),#Patient number
            ...
            HF_pacemaker=as.integer(any(HF_pacemaker==1)),
            HF_vad=as.integer(any(HF_vad==1))
