...
condition_prefix="IHD_op"
dataspec_prefix="acute IHD revasc procedures"
top_dir="../.."
...
codes=c("K65","K631","K632","K633","K634","K635","K636","K49","K50","K75","K40","K41","K42","K43","K44","K45","K46")
names(codes)=c(rep("ACS_pci",10),rep("ACS_cayg",7))
...
dat$hier_1=0
dat[dat$OPER_CD %in% c("K631", "K632", "K633", "K634", "K635", "K636"),]$"hier_1"=1
...
for (code in codes){
  if (TRUE %in% (startsWith(dat$OPER_CD,code))){ #If this code is present somewhere
    ...
    dat[startsWith(dat$OPER_CD,code),]$"hier_1"=3 #Then set the hierarchy for it 
...
dat_spell = 
  ...
  summarize(patient_number=first(PAT_ID_E),#Patient number
            ...
            ACS_pci=as.integer(any(ACS_pci==1)),
            ACS_cayg=as.integer(any(ACS_cayg==1))
