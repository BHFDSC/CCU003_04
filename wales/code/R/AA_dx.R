...
condition_prefix="AA_dx"
dataspec_prefix="Aortic_aneurysm_dx"
top_dir="../.."
...
#Keep only those prefixed with AA_
...
dat = dat %>%  arrange(SPELL_NUM_E,EPI_NUM,DIAG_NUM)
  
#Clean diagnosis codes of errors 
dat$DIAG_CD=str_replace(dat$DIAG_CD,"-","")
  
#######################################################
#Covid

#Get covid flag for diagnoses
covid_codes=c("B342","B972","U409","U071","U072") 
dat$"POSS_COVID"=as.integer(dat$DIAG_CD %in% covid_codes)

#######################################################
#Hierarchies

#For AA we have two hierarchies. 

#Check diagnoses for the first hierarchy
dat$hier_1=0
dat[dat$DIAG_CD %in% c("I710", "I711", "I713", "I715", "I718"),]$"hier_1"=1
dat[dat$DIAG_CD %in% c("I712", "I714", "I716", "I719")        ,]$"hier_1"=2


#Check for the second hierarchy
dat$hier_2=0
dat[dat$DIAG_CD %in% c("I710", "I718"),]$"hier_2"=1
dat[dat$DIAG_CD == "I711",]$"hier_2"=2
dat[dat$DIAG_CD == "I712",]$"hier_2"=3
dat[dat$DIAG_CD == "I713",]$"hier_2"=4
dat[dat$DIAG_CD == "I714",]$"hier_2"=5
dat[dat$DIAG_CD == "I715",]$"hier_2"=6
dat[dat$DIAG_CD == "I716",]$"hier_2"=7
dat[dat$DIAG_CD == "I719",]$"hier_2"=8

# Uncomment to see the amounts - good to check that each hierarchy is accounted for 
# table(dat$hier_1)
# table(dat$hier_2)
...
#Get the hierarchies for the diagnoses of the last episode of the spell
dat = dat %>% 
      group_by(SPELL_NUM_E,PROV_UNIT_CD,EPI_NUM) %>%  
      mutate(last_episode_hier_1=case_when(lastepisodeinspell==TRUE ~ min(hier_1[hier_1>0]),
             ...
             last_episode_first_diag_hier_1=case_when(lastepisodeinspell==TRUE & DIAG_NUM==1 ~ hier_1,
                                                      TRUE ~ NA_real_),
             last_episode_first_diag_hier_2=case_when(lastepisodeinspell==TRUE & DIAG_NUM==1 ~ hier_2,
                                                      TRUE ~ NA_real_))

#Get the diagnostic positions of the hierarchies for the first two categories (ie all episodes and last episode of spell)
dat = dat %>%  
      group_by(SPELL_NUM_E,PROV_UNIT_CD,EPI_NUM) %>% 
      mutate(diag_pos_all_ep_h1=case_when(hier_1==episode_hier_1 ~ DIAG_NUM,
                                          TRUE ~ NA_integer_),
             diag_pos_all_ep_h2=case_when(hier_2==episode_hier_2 ~ DIAG_NUM,
                                          TRUE ~ NA_integer_),
             diag_pos_last_ep_h1=case_when(hier_1==last_episode_hier_1 ~ DIAG_NUM,
                                          TRUE ~ NA_integer_),
             diag_pos_last_ep_h2=case_when(hier_2==last_episode_hier_2 ~ DIAG_NUM,
                                          TRUE ~ NA_integer_))
...
#Uncomment to check they're all accounted for
...
dat$"CHARLSON_CANCER"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="cancer",]$ICD10_CODE))
dat$"CHARLSON_CEREBRO"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="cerebrovascular disease",]$ICD10_CODE))
dat$"CHARLSON_CPD"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="chronic pulmonary disease",]$ICD10_CODE))
dat$"CHARLSON_CONGEST_HEART_FAIL"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="congestive heart failure",]$ICD10_CODE))
dat$"CHARLSON_DEMENTIA"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="dementia",]$ICD10_CODE))
dat$"CHARLSON_DIABETES_CHRON"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="diabetes with chronic complication",]$ICD10_CODE))
dat$"CHARLSON_DIABETES_WO_CHRON"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="diabetes without chronic complication",]$ICD10_CODE))
dat$"CHARLSON_HEMIPLEGIA_PARAPLEGIA"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="hemiplegia or paraplegia",]$ICD10_CODE))
dat$"CHARLSON_METASTATIC_CANCER"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="metastatic cancer",]$ICD10_CODE))
dat$"CHARLSON_MYOCARD_INFARCT"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="myocardial infarction",]$ICD10_CODE))
dat$"CHARLSON_PEPTIC_ULCER"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="peptic ulcer disease",]$ICD10_CODE))
# dat$"CHARLSON_PERIPH_VASC"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="peripheral vascular disease",]$ICD10_CODE))
dat$"CHARLSON_PERIPH_VASC"=0
dat$"CHARLSON_RENAL"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="renal disease",]$ICD10_CODE))
dat$"CHARLSON_RHEUMATIC"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="rheumatic disease",]$ICD10_CODE))
dat$"CHARLSON_SEVERE_LIVER"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="severe liver disease",]$ICD10_CODE))
dat$"CHARLSON_HIV"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="hiv",]$ICD10_CODE))
dat$"CHARLSON_MILD_LIVER"=as.integer(dat$DIAG_CD %in% unique(specdat[tolower(specdat$CHARLSON_DESC)=="mild liver disease",]$ICD10_CODE))
...
#Save the appropriate column name for the admission data (later on)
specdat$column=""
...
dat_spell = 
  ...
  summarize(patient_number=first(PAT_ID_E),#Patient number
            ...
            hier1pos=min(diag_pos_all_ep_h1,na.rm=T),
            
            #Repeat for the second hierarchy
            hier2=max(episode_hier_2),
            hier2pos=min(diag_pos_all_ep_h2,na.rm=T),
            
            #Last episode hierarchy 1
            last_ep_hier1=first(na.omit(last_episode_hier_1)),
            last_ep_hier1pos=first(na.omit(diag_pos_all_ep_h1)),

            #Repeat for hier 2
            last_ep_hier2=first(na.omit(last_episode_hier_2)),
            last_ep_hier2pos=first(na.omit(diag_pos_all_ep_h2)),
            
            #And the first diagnosis of the last episode
            last_ep_d1_h1=first(na.omit(last_episode_first_diag_hier_1,)),
            last_ep_d1_h2=first(na.omit(last_episode_first_diag_hier_2,)),
            
            #Get the ICD code for the first diagnosis of the last episode
            last_ep_d1_code=first(DIAG_CD[!is.na(last_episode_first_diag_hier_1)]),
